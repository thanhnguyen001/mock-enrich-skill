import React from "react";
import "./Header.scss";
import { CaretDownOutlined } from "@ant-design/icons";
import { Link } from "react-router-dom";
import { useAppSelector } from "hooks/hook";

const Header: React.FC = (props: any) => {
  const user = useAppSelector((state) => state.user);
  return (
    <div id="header" className="">
      <div className="header-wrapper h-full flex items-center justify-between sm:container sm:mx-auto">
        <div className="logo px-[4px] py-[4px] border rounded" id="logo">
          <p className="font-bold m-0">NEWS</p>
        </div>
        <div className="header-user flex items-center">
          {user && (
            <div className="header-user-avatar h-avatar w-avtar">
              <img
                src="data:image/png;base64,"
                alt=""
              />
            </div>
          )}
          {user && (
            <Link to={`/${user.ten_nhan_vien}`} className="header-user-name ml-4px flex items-center relative">
              <p className="m-0 font-semibold">{user.ten_nhan_vien}</p>
              {!user && <p className="m-0 font-semibold">Login</p>}
              <CaretDownOutlined className="ml-4px cursor-pointer" />
            </Link>
          )}
          {!user && (
            <Link to={"/log/login"} className="header-user-name ml-4px flex items-center relative">
              <p className="m-0 font-semibold">Login</p>
              <CaretDownOutlined className="ml-4px cursor-pointer" />
            </Link>
          )}
        </div>
      </div>
    </div>
  );
};

Header.propTypes = {};

export default Header;
