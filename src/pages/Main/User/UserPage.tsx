...
const UserPage: React.FC = () => {
  ...
  const handleUploadAvatar = async (e: ChangeEvent) => {
    const imgFile = (e.target as HTMLInputElement).files;
    if (imgFile && imgFile.length) {
      const f = new FormData();
      f.append("fileUpload", imgFile[0]);
      const res = (await mediaApi.uploadFile(f)) as unknown as IUploadFileRes;

      if (res?.fileUrl) {
        setAvatar(res.fileUrl);
        updateUser(res.fileUrl);
      }
    }
  };
  ...
  const updateUser = async (newAvatar: string) => {
    if (!user) return;
    const body: IUser = { ...user, anh_dai_dien: newAvatar };
    try {
      ...
      <div className="user-info py-[8px] w-full flex">
        <div className="user-intro justify-center items-center w-[35%] rounded-[5px]">
          <div className="flex relative">
            {user?.nhan_vien_id === userMain?.nhan_vien_id && (
              <div className="camera absolute bottom-[0px] left-[85px]  opacity-80 cursor-pointer">
